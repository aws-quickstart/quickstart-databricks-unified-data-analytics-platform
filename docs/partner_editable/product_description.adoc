...
** A security group protects individual control-plane hosts from the external internet, with the exception of inbound ports to the load balancer, which dispatches web-application and API requests to the appropriate internal services.
...
An Amazon Simple Storage Service (Amazon S3) bucket is created in the customer account with default encryption and bucket-versioning enabled. Amazon S3 blocks all public access and, using a lifecycle management rule, permanently deletes deprecated versions after five days.
