...
** Databricks clusters of Amazon Elastic Compute Cloud (Amazon EC2) instances. This Quick Start deploys one workspace and sets up the environment for deploying more workspaces in the future.
...
* If you provide one, a customer-managed AWS Key Management Service (AWS KMS) key to encrypt notebooks.
...
* A cross-account AWS Identity and Access Management (IAM) role to enable Databricks to deploy clusters in the VPC for the new workspaces. Depending on the deployment option you choose, you either create this IAM role during deployment or use an existing IAM role.
